...
Hopefully, I stood up all night (you know being part of the organization team etc) so I started hacking near by my friend @phillipeAntoine, who was tinkering on his yet to be released canvas + css demo. 
...
The problem is the markup, not the CSS. HTML is very verbose even for small stuff. That's why this demo uses some javascript to generate markup. This way, I am able to compose from a simple string lots of elements that will be used as part of the animation. The recipe : 

# take a sentence 
# split it et get an array
# apply a function on each element (here forEach but map would have saved me some characters) 

In action : 

bc. "Hi DemoJS partiers and orga! Thx 4 being awesome! CSS 
...
bc. document.getElementById("z").className='s'

h3. Final word 

The code is ugly, the demo is not very impressive but when looking back, it does contain some stuff, I hope, you find instructing :D
